...
const CommercialPropertyForm = ({ onPropertyAdded }) => {
  ...
  // REFERENCIA PARA AUTOCOMPLETE
  const autocompleteRef = useRef(null);
  const inputRef = useRef(null);

  useEffect(() => {
    // Carga de ciudades y tipos de propiedad igual que antes
    ...
    // Configurar Autocomplete Google Maps para el input de ubicación
    if (window.google && window.google.maps && window.google.maps.places) {
      autocompleteRef.current = new window.google.maps.places.Autocomplete(inputRef.current, {
        types: ["geocode"], // Podés usar "address" también
    ...
    <Box sx={{ width: "100%" }}>
      ...
      {activeStep === 0 ? (
        <Box sx={{ mb: 2 }}>
          ...
          <TextField
            inputRef={inputRef} // <=== Acá va la referencia para autocomplete
            name="ubicacion"
            label="Ubicación"
            value={propertyData.ubicacion}
            onChange={(e) =>
              setPropertyData((prev) => ({ ...prev, ubicacion: e.target.value }))
            }
            required
            fullWidth
            margin="normal"
            ...
            {caracteristicasLista.map((caracteristica) => (
              <FormControlLabel
                ...
                label={
                  ...
                  caracteristica.slice(1).replace(/([A-Z])/g, " $1")
                }
              />
            ))}
          </FormGroup>
